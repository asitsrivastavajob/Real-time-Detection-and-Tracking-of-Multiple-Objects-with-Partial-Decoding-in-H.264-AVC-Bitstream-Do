...
void motion_segment()
{
		...
		if(trajectory_optimization_flag==1)
		{
			if(motion_picture_num>=intra_period)
			{
				...
				*/
				...
				for(i=0; i<max_buf_group_num; i++)		//현재 프레임의 각 그룹에 대하여
				{
					//객체 그룹에 대하여 FrameCnt를 증가시킨다.
					...
					if((gbuf[i].Status==OBJECT_GROUP)&&(gbuf[i].ObjectFrame+gbuf[i].FrameCnt>=intra_period))
					{
						...
						//객체 컬러정보 추출
						//1. 예측된 객체 영역에서 배경 제거 (background subtraction)
						...
						//2. 객체 픽셀 위치의 최대, 최소값 추출 후 객체 영역 최적화 (region optimization)
						max_obj_x = min_x-1;
						min_obj_x = max_x+1;
						max_obj_y = min_y-1;
						min_obj_y = max_y+1;
						...
						//이전 GOP에서의 객체 위치 및 영역 계산(motion interpolation in P frmaes of the previous GOP)
				...
				//이전 GOP에 대한 결과 동영상 쓰기 (drawing the motion trajectory)
	...
	else		//P-frames
	{
		...
		//-------------------------------------------------------------------------------------------------------
		//
		//인터 매크로블록의 추출 (select inter blocks) : delete skip blocks
		//
		//-------------------------------------------------------------------------------------------------------
		...
		//-------------------------------------------------------------------------------------------------------
		//
		//매크로블록의 그룹핑 (macroblock grouping) : block group labeling
		//
		//-------------------------------------------------------------------------------------------------------
		...
		do
		{
			...
			for(k=0; k<max_mb_nr; k++)
			{
				if(mblabel[motion_picture_num][k]>=0)	//no SKIP block
				{
					...
					{
						mblabel[motion_picture_num][k] = -1;		//고립된 매크로블록의 레이블을 SKIP 모드로 전환한다.
						rejected_group_num++;
					}
		...
		//-------------------------------------------------------------------------------------------------------
		//
		//객체 그룹의 인식과 에러 그룹의 제거 (Spatial and Temporal Filtering)
		//
		//-------------------------------------------------------------------------------------------------------
		...
		if(object_recognition_flag==1)
		{
			...
			for(i=0; i<MAX_BUF_NUM; i++)
			{
				...
				tmp_gbuf_Status[i] = gbuf[i].Status;		//이전 프레임에서 각 버퍼의 상태
				if((gbuf[i].CheckCnt>=0)&&((gbuf[i].Status==CANDIDATE_GROUP)||(gbuf[i].Status==OBJECT_GROUP)))
					tmp_gbuf_FrmGroupIdx[i] = gbuf[i].FrmGroupIdx[gbuf[i].CheckCnt];		//이전 프레임에서 각 프레임 그룹 인덱스
				else
			...
			for(i=0; i<UNKNOWN_LABEL; i++)        //현재 프레임의 각 그룹에 대하여 (i => block group label)
			{
				if(label_size[i]>0)
				{
					overlap_check_flag = 0;		//해당 그룹에 일치하는 버퍼 주소가 있는지 여부를 체크한다.
					curr_buf_address = 0;
					pros_buf_address = -1;
					while((overlap_check_flag!=1)&&(curr_buf_address<prev_max_buf_group_num))        //체크된 그룹이 없고 버퍼의 모든 주소가 아직 검색되지 않는 동안
					{
						   if(((tmp_gbuf_Status[curr_buf_address]==CANDIDATE_GROUP)||(tmp_gbuf_Status[curr_buf_address]==OBJECT_GROUP))&&(gbuf[curr_buf_address].CheckCnt>=0))        //버퍼의 주소가 추천 그룹 또는 객체 그룹이라면 (block group is already Candidate or Object group)
						   {
								  //현재 프레임의 그룹과 겹치는 부분이 있는지 체크한다.
								  overlap_mb_cnt = 0;
								  for(m=0; m<max_mb_nr; m++)        //현재 프레임의 그룹 내의 모든 매크로블록에 대하여
								  {
										 if((mblabel[motion_picture_num][m]==i)&&(mblabel[motion_picture_num-prev_dist][m]==tmp_gbuf_FrmGroupIdx[curr_buf_address]))      //현재 프레임에서 해당 매크로블록의 레이블이 현재 버퍼 주소의 그룹 레이블과 일치하다면
											   overlap_mb_cnt++;
								  }

								  if(overlap_mb_cnt>0)        //겹치는 부분이 있다면 (if overlapping)
								  {
										 ...
										 if((gbuf[curr_buf_address].ChkOn==1)&&(gbuf[curr_buf_address].DominantGroupIdx>=0)&&(gbuf[curr_buf_address].DominantGroupIdx!=i)
											 &&(gbuf[curr_buf_address].CheckCnt>0))        //버퍼의 주소가 체크된 주소라면
										 {
											   for(m=0; m<max_mb_nr; m++)        //현재 프레임에서 그룹 내의 모든 매크로블록에 대하여
											   {
													 if(mblabel[motion_picture_num][m]==i)
														   mblabel[motion_picture_num][m] = gbuf[curr_buf_address].DominantGroupIdx;      //현재 프레임의 이 그룹에 포함된 모든 매크로블록의 레이블을 겹쳐진 기존 그룹의 최우선 레이블 인덱스로 전환한다.
											   }

											   assigned_buf_idx[i] = curr_buf_address;		//현재 그룹에 할당된 버퍼 그룹의 주소
										 ...
										 else      //버퍼의 주소가 체크된 주소가 아니라면
										 {
												gbuf[curr_buf_address].DominantGroupIdx = i;      //현재 프레임의 그룹을 현재 버퍼 주소에 대응하는 최우선 레이블 인덱스로 선정한다.
												assigned_buf_idx[i] = curr_buf_address;		//현재 그룹에 할당된 버퍼 그룹의 주소
												(gbuf[curr_buf_address].CheckCnt)++;      //check Counter를 업데이트한다.
										 ...
										 //현재 프레임의 그룹이 버퍼의 그룹과 동일한 레이블을 지니는 것으로 판단하고, 버퍼의 그룹에 대한 파라메터를 업데이트한다.
										 ...
										 if(gbuf[curr_buf_address].CheckCnt<=decision_window_size)
										 {
											 if(gbuf[curr_buf_address].MbNum[gbuf[curr_buf_address].CheckCnt-1]>0)
													gbuf[curr_buf_address].OccurProb[gbuf[curr_buf_address].CheckCnt] = (double)overlap_mb_cnt / (double)gbuf[curr_buf_address].MbNum[gbuf[curr_buf_address].CheckCnt-1];      //occurrence probability를 업데이트한다.
										 }
										 gbuf[curr_buf_address].FrmGroupIdx[gbuf[curr_buf_address].CheckCnt] = gbuf[curr_buf_address].DominantGroupIdx;      //frame group pointer를 업데이트한다.
										 gbuf[curr_buf_address].ChkOn = 1;                                   //group check status를 업데이트한다.
										 gbuf[curr_buf_address].OccurCnt++;

										 if((tmp_gbuf_Status[curr_buf_address] == CANDIDATE_GROUP)&&(gbuf[curr_buf_address].CheckCnt==decision_window_size))	//window 간격 사이의 모든 프레임이 체크된 경우,
										 {
											 //total occurrence probability를 계산한다.
											  total_occur_prob = 0;
											  ...
											  else
											  {
													 ...
													 //윈도우 전체 프레임에 대하여, 이 에러 그룹에 포함된 모든 매크로블록의 레이블을 초기화한다.
										 ...
										 if(gbuf[curr_buf_address].Status==OBJECT_GROUP)
										 {
											 ...
											 if(mb_cnt>0)
											 {
													 gbuf[curr_buf_address].Width[gbuf[curr_buf_address].FrameCnt] = (max_mbx - min_mbx +1)*16;           //group size의 width를 업데이트한다.
													 gbuf[curr_buf_address].Height[gbuf[curr_buf_address].FrameCnt] = (max_mby - min_mby +1)*16;          //group size의 height를 업데이트한다.

													 if(gbuf[curr_buf_address].FrameCnt==0)
													 {
														 gbuf[curr_buf_address].PosX[gbuf[curr_buf_address].FrameCnt] = TmpX*16 / mb_cnt +8;         //group position x좌표를 업데이트한다.
														 gbuf[curr_buf_address].PosY[gbuf[curr_buf_address].FrameCnt] = TmpY*16 / mb_cnt +8;         //group position x좌표를 업데이트한다.
													 }
						   ...
						   else        //버퍼의 주소가 미사용 상태라면
						   {
								  if(pros_buf_address<0)        //최우선 버퍼 주소가 설정되어 있지 않다면
										 pros_buf_address = curr_buf_address;        //새로운 그룹이 등록될 때 위치할 최우선 버퍼 주소(pros_buf_address)를 설정한다.
						   }

						   curr_buf_address++;
					}

					if(overlap_check_flag==0)      //체크된 주소가 없다면 (즉, 오버랩된 주소가 없다; if not overlapped)
					{
						  //프레임의 현재 그룹을 버퍼의 새로운 주소에 추천 그룹으로 등록하고 관련 파라메터를 설정한다.
						  ...
						  gbuf[new_buf_address].DominantGroupIdx = i;      //현재 프레임의 그룹을 현재 버퍼 주소에 대응하는 최우선 레이블 인덱스로 선정한다.
						  ...
						  if(mb_cnt>0)
						  {
								  gbuf[new_buf_address].PosX[0] = TmpX*16 / mb_cnt +8;         //group position x좌표를 업데이트한다.
								  gbuf[new_buf_address].PosY[0] = TmpY*16 / mb_cnt +8;         //group position x좌표를 업데이트한다.								  
						  }
						  gbuf[new_buf_address].Width[0] = (max_mbx - min_mbx +1)*16;           //group size의 width를 업데이트한다.
						  gbuf[new_buf_address].Height[0] = (max_mby - min_mby +1)*16;          //group size의 height를 업데이트한다.	
						  */

						  gbuf[new_buf_address].ChkOn = 1;                                   //group check status를 업데이트한다.
			...
			//for(m=0; m<max_mb_nr; m++)
			//	buf_mblabel[m] = mblabel[motion_picture_num][m];	//이전 P 프레임에서의 매크로블록 레이블

			//Check Block Group Buffer (BGB)

			for(i=0; i<max_buf_group_num; i++)        //현재 프레임의 각 그룹에 대하여 (i => block group buffer label)
			{
				  if(gbuf[i].ChkOn!=1)
				  {
					  if((tmp_gbuf_Status[i] == CANDIDATE_GROUP)||(tmp_gbuf_Status[i] == OBJECT_GROUP))
					  {
						  ...
						  for(m=0; m<max_mb_nr; m++)        //현재 프레임의 그룹 내의 모든 매크로블록에 대하여
						  {
								 ...
								 {
									 if((mblabel[motion_picture_num-prev_dist][m]==tmp_gbuf_FrmGroupIdx[i])&&(assigned_buf_idx[mb_label]!=i)&&(assigned_buf_idx[mb_label]>=0))      //버퍼 그룹이 현재 그룹과 overlap되며, 기존의 할당된 버퍼 그룹과 다르다면,
									 {
						  ...
						  if((overlap_buf_group_idx>=0)&&(!((gbuf[overlap_buf_group_idx].Status==OBJECT_GROUP)&&(tmp_gbuf_Status[i]==OBJECT_GROUP)))
							  &&(gbuf[overlap_buf_group_idx].Status!=ERROR_GROUP)&&(tmp_gbuf_Status[i]!=ERROR_GROUP))        //겹치는 부분이 있되, 이전 그룹과 overlap 그룹이 한꺼번에 객체 그룹이 되지 않는 경우,
						  {
							  if(tmp_gbuf_Status[i]==CANDIDATE_GROUP)		//이전 그룹이 추천 그룹이라면
							  {
								  //이전 그룹을 에러 그룹으로 처리한다.
							  ...
							  {
								  if((gbuf[overlap_buf_group_idx].Status==CANDIDATE_GROUP)&&(gbuf[i].CheckCnt>=0)&&(gbuf[overlap_buf_group_idx].CheckCnt>=0))		//이전 그룹이 객체 그룹이고, overlap 그룹이 추천 그룹이라면
								  {
									  //기존 overlap 그룹이 갖고 있는 모든 파라메터를 그대로 이전 그룹으로 복사한다.									  
									  (gbuf[i].CheckCnt)++;
									  (gbuf[i].FrameCnt)++;
									  ...
									  //{
											 ...
											 if(mb_cnt>0)
											 {
													 gbuf[i].Width[gbuf[i].FrameCnt] = (max_mbx - min_mbx +1)*16;           //group size의 width를 업데이트한다.
													 gbuf[i].Height[gbuf[i].FrameCnt] = (max_mby - min_mby +1)*16;          //group size의 height를 업데이트한다.

													 if(gbuf[i].FrameCnt==0)
													 {
														 gbuf[i].PosX[gbuf[i].FrameCnt] = TmpX*16 / mb_cnt +8;         //group position x좌표를 업데이트한다.
														 gbuf[i].PosY[gbuf[i].FrameCnt] = TmpY*16 / mb_cnt +8;         //group position x좌표를 업데이트한다.
													 }
						  ...
						  else		// 겹치는 부분이 없거나, 이전 그룹과 overlap 그룹이 모두 객체 그룹인 경우,
						  {
							  if(gbuf[i].CheckCnt>=0)
							  {
								  (gbuf[i].CheckCnt)++;      //check Counter를 업데이트한다.
								  if(tmp_gbuf_Status[i] == OBJECT_GROUP)
									  (gbuf[i].FrameCnt)++;

								  //임시 매크로블록 레이블 버퍼에 가상의 매크로블록 그룹을 포함시킨다.
								  max_label_idx++;
								  cnt=0;
								  for(m=0; m<max_mb_nr; m++)        //현재 프레임의 그룹 내의 모든 매크로블록에 대하여
								  {
								  ...
								  gbuf[i].FrmGroupIdx[gbuf[i].CheckCnt] = max_label_idx;      //frame group pointer를 업데이트한다.
								  gbuf[i].MbNum[gbuf[i].CheckCnt] = gbuf[i].MbNum[gbuf[i].CheckCnt-1];
								  if(gbuf[i].CheckCnt<=decision_window_size)
										gbuf[i].OccurProb[gbuf[i].CheckCnt] = 1;      //occurrence probability를 업데이트한다.								  
								  gbuf[i].ChkOn = 1;                                   //group check status를 업데이트한다.

								  if((tmp_gbuf_Status[i] == CANDIDATE_GROUP)&&(gbuf[i].CheckCnt==decision_window_size))	//window 간격 사이의 모든 프레임이 체크된 경우,
								  {
									   //total occurrence probability를 계산한다.
									   total_occur_prob = 0;
									   for(n=1; n<=decision_window_size; n++)
								  ...
								  if(tmp_gbuf_Status[i]==OBJECT_GROUP)
								  {
									  gbuf[i].PosX[gbuf[i].FrameCnt] = gbuf[i].PosX[gbuf[i].FrameCnt-1];         //group position x좌표를 업데이트한다.
									  gbuf[i].PosY[gbuf[i].FrameCnt] = gbuf[i].PosY[gbuf[i].FrameCnt-1];         //group position x좌표를 업데이트한다.
									  gbuf[i].Width[gbuf[i].FrameCnt] = gbuf[i].Width[gbuf[i].FrameCnt-1];           //group size의 width를 업데이트한다.
									  gbuf[i].Height[gbuf[i].FrameCnt] = gbuf[i].Height[gbuf[i].FrameCnt-1];          //group size의 height를 업데이트한다.
								  }
								  /*
								  else
								  {
